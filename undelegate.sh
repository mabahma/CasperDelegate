...
echo -e "\n${GREEN}Please enter the address (public key) of the validator you would like to undelegate from.${NC}"
...
echo -e "${GREEN}Amount to be undelegated: $UNDELEGATE_AMOUNT ${NC}"
echo -e "${GREEN}Amount to be undelegated in motes: $UNDELEGATE_MOTES ${NC}"
...
$casper-client put-deploy --chain-name casper --node-address http://188.165.254.13:7777 -k $HOME/privkey.pem --session-path "$HOME/casper-node/target/wasm32-unknown-unknown/release/undelegate.wasm" --payment-amount 3000000000  --session-arg "validator:public_key=’$VALIDATOR_PUBKEY'" --session-arg="amount:u512='$UNDELEGATE_MOTES'" --session-arg "delegator:public_key='$DELEGATOR_PUBKEY'"
